...
class Controller_Client_PlannedOrder extends AC_Controller
{
	...
	public function action_tree(){
		$data =	Jelly::factory('client_work_plannedorder')->get_tree();
	...
	public function action_status_list(){
		$data =	Jelly::factory('client_work_plannedorder')->get_status_tree();
		$this->request->response = Json::arr($data, count($data));
	}
	
	public function action_fields_list(){
		$user = Auth::instance()->get_user();
		$data =	Jelly::factory('client_work_plannedorder')->get_fields_tree($user->license->id());
		$this->request->response = Json::arr($data, count($data));
	}
	...
	public function action_table(){
		...
		$view->data = Jelly::factory('client_work_plannedorder')->get_table_data($filter);
	...
	public function action_edit($id = null, $read = false, $create_data = false){
        ...
        if($id){
            $model = Jelly::select('client_work_plannedorder')->where(':primary_key', '=', (int)$id)->load();
		...
		$view = Twig::factory('client/work/planned_order');

		if($model){//если просмотр или редактирование
			$view->model = $model->as_array();
			$view->model['fields'] = Jelly::factory('client_work_plannedorder')->get_fields_data((int)$id);
        }
...
//		print_r($data); exit;
		...
		foreach($data as $row){	
			...
			if(UTF8::strpos($row['rowId'], 'porder') !== false){
				...
				if($id=='new'){
					$id = false;
					$model = Jelly::factory('client_work_plannedorder');
				}else{
					$model = Jelly::select('client_work_plannedorder', (int)$id);
			...
			if(UTF8::strpos($row['rowId'], 'field') !== false){
				...
				if(UTF8::strpos($field_id, 'new') !== false){
					$field_id = false;
					$field_model = Jelly::factory('client_work_plannedorder2field');
				}else{
					$field_model = Jelly::select('client_work_plannedorder2field', (int)$field_id);
				...
				$field_model->planned_order = $order_id;
			...
			if(UTF8::strpos($row['rowId'], 'materials') !== false && UTF8::strpos($row['rowId'], 'title') === false){
				...
				Jelly::factory('Client_Work_PlannedOrderField2Material')->save_from_grid($materials_data, $order_id, $field_id);
			}
			...
			if(UTF8::strpos($row['rowId'], 'technics') !== false && UTF8::strpos($row['rowId'], 'title') === false){
				...
				Jelly::factory('Client_Work_PlannedOrderField2Technic')->save_from_grid($technics_data, $order_id, $field_id);
			}
			...
			if(UTF8::strpos($row['rowId'], 'personals') !== false && UTF8::strpos($row['rowId'], 'title') === false){
				...
				Jelly::factory('Client_Work_PlannedOrderField2Personal')->save_from_grid($personals_data, $order_id, $field_id);
			}
			
		}
		
		if(count($not_delete_fields)) Jelly::delete('client_work_plannedorder2field')->where('planned_order', '=', $order_id)->and_where('_id', 'NOT IN', $not_delete_fields)->execute();
		else						  Jelly::delete('client_work_plannedorder2field')->where('planned_order', '=', $order_id)->execute();
