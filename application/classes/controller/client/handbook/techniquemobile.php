...
class Controller_Client_Handbook_TechniqueMobile extends Controller_Glossary_Abstract
{
	protected $model_name = 'client_handbook_techniquemobile';
	protected $model_group_name = 'client_handbook_techniquemobilegroup';
	...
	public function action_update(){
		...
		$parent_group_in_handbook = Jelly::select('client_handbook_techniquemobilegroup')->
...
//				where('farm','=',$farm)->
		...
		$parent_group_in_glossary = Jelly::select('glossary_techmobile_group')->where('_id','=',$parent_group_in_handbook->id_in_glossary)->execute();
		...
		if ($parent_group_in_handbook && $parent_group_in_handbook->_id>0) {
			...
			$new_parent_group_in_handboook = Jelly::factory('client_handbook_techniquemobilegroup');
	...
	public function inner_update($contragent_id){
		...
		Jelly::factory('extraproperty')->updateOldFields((int)$contragent_id, 'handbooktechmobile_chief', $addons);
		...
		Jelly::factory('extraproperty')->updateOldFields((int)$contragent_id, 'handbooktechmobile_end', $addons);
	...
	public function action_edit($id = null, $read = false, $parent_id = false){
        ...
        if($id){
            $model = Jelly::select('client_handbook_techniquemobile')->with('group')->where(':primary_key', '=', (int)$id)->load();
		...
		$view = Twig::factory('client/handbook/techniquemobile/read_techniquemobile');
        ...
        if(!$read){
            ...
            if((int)$parent_id)
            {
					$parent = Jelly::select('client_handbook_techniquemobilegroup', (int)$parent_id);
                    $view->model                           = array();
        ...
        if($model){
			...
			$view->model['chief_properties']  = Jelly::select('extraproperty')->where('object', '=', $model->id())->and_where('block', '=', 'handbooktechmobile_chief')->execute()->as_array();
			$view->model['end_properties']  = Jelly::select('extraproperty')->where('object', '=', $model->id())->and_where('block', '=', 'handbooktechmobile_end')->execute()->as_array();
        }
		...
		if(count($s_farms) or count($s_farm_groups))
	   {
        ...
        if($selected_farm != '')
        {
            $selected_farm_obj = Jelly::select('farm')->
					where_open()->where('deleted', '=', 0)->or_where('deleted', 'IS', null)->where_close()->
					and_where('license', '=', $user->license->id())->load((int)$selected_farm);
	...
	public function inner_edit(&$view){
		...
		$view->max_power_units = Jelly::factory('glossary_units')->getUnits('max_power');
		$view->work_volume_units = Jelly::factory('glossary_units')->getUnits('work_volume');
		$view->fuel_cosumption_units = Jelly::factory('glossary_units')->getUnits('fuel_cosumption');
		$view->tank_capacity_units = Jelly::factory('glossary_units')->getUnits('tank_capacity');
		$view->oil_change_period_units = Jelly::factory('glossary_units')->getUnits('oil_change_period');
	...
	public function action_data($item_id){
		$data = Jelly::select('client_handbook_techniquemobile')->with('group')->where(':primary_key', '=', (int)$item_id)->load();
		$data = $data->as_array();
