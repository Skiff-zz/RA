...
class Controller_Client_Handbook_TechniqueMobileGroup extends Controller_Glossary_AbstractGroup
{
	...
	public function action_delete($id = null){
		...
		for($i=0; $i<count($del_ids); $i++){
			...
			$m = mb_substr($del_ids[$i], 0, 1)=='n' ? 'client_handbook_techniquemobile' : 'client_handbook_techniquemobilegroup';
			...
			if ($id==-2 || $id=='-2') {
				...
				$names_to_delete = Jelly::select('client_handbook_techniquemobile')->where('group','=',$model->id())->where('license', '=', $license_id)->execute();
	...
	public function action_edit($id = null, $read = false, $parent_id = false){
        ...
        if($id && $id!=-2){
            $model = Jelly::select('client_handbook_techniquemobilegroup')->with('parent')->where(':primary_key', '=', (int)$id)->load();
		...
		$view = Twig::factory('client/handbook/techniquemobile/read_techniquemobilegroup');
        ...
        if($model){
			$view->model['properties']  = Jelly::select('extraproperty')->where('object', '=', $model->id())->and_where('block', '=', 'handbooktechniquemobilegroup_prop')->execute()->as_array();
        }
