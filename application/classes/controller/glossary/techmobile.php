...
class Controller_Glossary_TechMobile extends Controller_Glossary_Abstract
{
	...
	public function action_data($item_id){
		$data = Jelly::select('glossary_techmobile')->with('group')->where(':primary_key', '=', (int)$item_id)->load();
		$data = $data->as_array();
	...
	public function inner_update($contragent_id){
		...
		$contragent = Jelly::select('glossary_techmobile', (int)$contragent_id);
    ...
    public function action_tree(){
        ...
        if($extras) foreach($data as &$record) $record['extras'] = Jelly::factory('client_transaction')->get_extras('glossary_techmobile', substr($record['id'], 1));
