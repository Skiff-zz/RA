...
class Controller_Glossary_TechTrailerGroup extends Controller_Glossary_AbstractGroup
{
	...
	public function inner_update($id){
		...
		$model = Jelly::select('glossary_techtrailergroup', (int) $id)->as_array();
		$handbook_siblings = Jelly::select('client_handbook_techniquetrailergroup')->
	...
	public function action_treeforhandbooktechniquetrailer(){
		...
		$in_handbook = Jelly::select('client_handbook_techniquetrailer')->
				with('group')->
				where_open()->where('deleted', '=', 0)->or_where('deleted', 'IS', null)->where_close()->
...
//				unset($data[$i]);
		...
		$in_handbook_groups = Jelly::select('client_handbook_techniquetrailergroup')->
				where_open()->where('deleted', '=', 0)->or_where('deleted', 'IS', null)->where_close()->
