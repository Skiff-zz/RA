...
class Model_Client_Work_HandOrderFieldTechnicAggregateBlock extends Jelly_Model
{

    
    public static function initialize(Jelly_Meta $meta)
	{
		$meta->table('work_handorderfieldtechnic_aggregate')
			->fields(array(
	...
	public function save_aggregates($second_grid_items, $main_grid_items, $order_id, $field_id, $hand_order_field_technic_id){
		$do_not_delete = array();
		...
		foreach($second_grid_items as $second_grid_item){
			...
			$item = Jelly::select('client_work_handorderfieldtechnicaggregateblock')->where('hand_order_field_technic', '=', $hand_order_field_technic_id)
			...
			if(!$item instanceof Jelly_Model || !$item->loaded()){
				$item = Jelly::factory('client_work_handorderfieldtechnicaggregateblock');
			}
		...
		if(count($do_not_delete)) Jelly::delete('client_work_handorderfieldtechnicaggregateblock')->where('hand_order_field_technic', '=', $hand_order_field_technic_id)->and_where('_id', 'NOT IN', $do_not_delete)->execute();
		else					  Jelly::delete('client_work_handorderfieldtechnicaggregateblock')->where('hand_order_field_technic', '=', $hand_order_field_technic_id)->execute();
	}
