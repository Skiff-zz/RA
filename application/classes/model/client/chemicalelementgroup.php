...
class Model_Client_ChemicalelementGroup extends Model_Glossary_AbstractGroup
{
	public static function initialize(Jelly_Meta $meta, $table_name = 'client_chemicalelementgroup', $items_model = 'client_chemicalelement')
	{
	...
	public function get_tree($license_id, $with_cultures = false, $exclude = array(), $items_field = 'items'){
		...
		$names = Jelly::select($items_model_name)->with($items_model->foreign['column'])->where('deleted', '=', false)->order_by('name', 'asc')->execute()->as_array();
			...
			for($i=count($this->result)-1; $i>=0; $i--){
				...
				foreach($names as $name) {
					...
					if($name[':'.$items_model->foreign['column'].':_id']==$group_id) {
						$group_names[] = array(
							'id'	   => 'n'.$name['_id'],
							'title'    => $name['name'].'</div>  <div style="color: #666666; height: 28px; padding-top:3px; padding-right:4px;">'.$name['symbol'].'</div><div>',
							'is_group' => true,
