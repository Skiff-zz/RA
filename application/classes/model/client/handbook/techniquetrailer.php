...
class Model_Client_Handbook_TechniqueTrailer extends Model_Glossary_Abstract
{
	protected $model_name = 'client_handbook_techniquetrailer';
    
    public static function initialize(Jelly_Meta $meta, $table_name  = 'client_handbook_techniquetrailer', $group_model = 'client_handbook_techniquetrailergroup')
	{
		...
		$meta->table($table_name)
			->fields(array(
				...
				'soil_depth' => Jelly::field('String', array('label' => 'Глубина обработки почвы')),
				'soil_depth_units'  => Jelly::field('BelongsTo',array(
						'foreign'	=> 'glossary_units',
						'column'	=> 'soil_depth_units_id',
						'label'		=> 'Единицы измерения'
				)),
				...
				'wheels_number' => Jelly::field('String', array('label' => 'Количество колес')),
				'axis_number' => Jelly::field('String', array('label' => 'Количество осей')),

				'lift_capacity' => Jelly::field('String', array('label' => 'Грузоподъемность')),
				'lift_capacity_units'  => Jelly::field('BelongsTo',array(
						'foreign'	=> 'glossary_units',
						'column'	=> 'lift_capacity_units_id',
						'label'		=> 'Единицы измерения'
				)),

				'weight' => Jelly::field('String', array('label' => 'Масса')),
				'weight_units'  => Jelly::field('BelongsTo',array(
						'foreign'	=> 'glossary_units',
						'column'	=> 'weight_units_id',
						'label'		=> 'Единицы измерения'
				)),
				'length' => Jelly::field('String', array('label' => 'Длина')),
				'length_units'  => Jelly::field('BelongsTo',array(
						'foreign'	=> 'glossary_units',
						'column'	=> 'length_units_id',
						'label'		=> 'Единицы измерения'
				)),
	...
	public function add_nomenclature($model, $model_ids, $license_id, $farm_id){
		...
		foreach($model_ids as $id){
			...
			$record = Jelly::select('glossary_techtrailer',(int)$id);
			$parent = Jelly::select('client_handbook_techniquetrailergroup')->
            ...
            $record_to = Jelly::factory('client_handbook_techniquetrailer')->set($buff)->save();
