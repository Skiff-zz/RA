...
class Model_Client_Handbook_TechniqueTrailerGroup extends Model_Glossary_AbstractGroup
{
	public static function initialize(Jelly_Meta $meta, $table_name = 'client_handbook_techniquetrailergroup', $items_model = 'client_handbook_techniquetrailer')
	{
	...
	public function add_nomenclature($model, $model_ids, $license_id, $farm_id){
		...
		foreach($new_root_elements as $id){
			...
			$record_to = Jelly::select('client_handbook_techniquetrailergroup')
...
//					->where('farm', '=', $farm_id)
			...
			if ($record_to && $record_to->id()) {
				...
				if ($parent){
					...
					$parent_group = Jelly::select('client_handbook_techniquetrailergroup')->
...
//							where('farm', '=', $farm_id)->
				...
				$host_record = Jelly::factory('client_handbook_techniquetrailergroup')->set($buff)->save();
			}
		}
