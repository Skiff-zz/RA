...
class Migration_Jelly extends Migration {
	...
	{
	  $model = (is_string($model_name))?Jelly::meta($model_name):$model_name;
		
		// If the model is an object instance of Jelly
		if (is_object($model) AND $model instanceof Jelly_Meta)
		{
	...
	protected function _tables()
	{
		...
		foreach ($this->_model->fields() as $field)
		{
			...
			elseif ($field instanceof Jelly_Field_ManyToMany)
			{
				...
				if(Jelly::meta($field->through['model'])!==false){
					continue;
				}
	...
	protected function _db()
	{
		...
		if (!$this->_model)
		{
		  ...
		  echo '</pre>';
		  throw new Kohana_Exception('Unexpected db', array('db' => $this->_model));
//      die();
		}
	...
	private function _columns(Jelly_Field $field, Database_Table $table)
	{
		...
		switch (TRUE) 
		{
			...
			case ($field instanceof Jelly_Field_Timestamp):
				...
				break;
			case ($field instanceof Jelly_Field_Datetime):
				$column = Database_Column::factory('date');
				break;	
				// Floats
				// Relationships
